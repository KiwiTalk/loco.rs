...
async fn register_device() {
    let result = TokenClient::new(Os::Win32).register_device(&get_device_register_data()).await;
    let text = result.unwrap();
    println!("{}", text.text().await.unwrap());
}
...
async fn request_passcode() {
    let result = TokenClient::new(Os::Win32).request_passcode(&get_device_register_data()).await;
    let text = result.unwrap();
    println!("{}", text.text().await.unwrap());
}
